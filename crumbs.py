...
from iso import world_velocity_for_screen_velocity
...
def spawn_bite_crumbs(
    ...
    if bite < 0 or bite >= len(BITE_CRUMBS):
        return
    wx, wy = wrap_pos(wx, wy)
    for quarter, svx, svy in BITE_CRUMBS[bite]:
        _append_crumb(crumbs, wx, wy, kind, quarter, svx, svy, cam_deg, scale=1.0, speed_mult=1.0, angle_jitter=0.0)
